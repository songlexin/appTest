...
export const useTabsViewStore = defineStore({
  ...
  getters: {
    getTabsList: (state) => {
      return state.tabsList.filter((item) => {
        return !item.meta?.hideInTabs && router.hasRoute(item.name!) && item.path != '/home/index';
      });
    },
    /** 当前activity tab */
    getCurrentTab: (state) => {
      const currentRoute = router.currentRoute.value!;
      const arr = state.tabsList.find((item) => {
        return !item.meta?.hideInTabs && item.path === currentRoute.path;
      });
      return arr;
    },
  },
  actions: {
    ...
    closeOtherTabs(route) {
      const targetIndex = this.tabsList.findIndex((item) => item.name === route.name);
      if (targetIndex !== -1) {
    ...
    closeCurrentTab(route) {
      ...
      if (isDelCurrentTab) {
        ...
        if (index == this.tabsList.length) {
          currentRoute = this.tabsList[Math.max(0, this.tabsList.length - 1)];
        } else {
          currentRoute = this.tabsList[index];
        }
